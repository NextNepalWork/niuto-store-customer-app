...
class WishlistProductsBloc
    extends Bloc<WishlistDetailEvent, WishlistProductsState> {
  final WishlistRepo wishlistRepo;

  WishlistProductsBloc(this.wishlistRepo)
      : super(const WishlistProductsState(
            status: WishlistProductsStatus.initial,
            hasReachedMax: false,
            wishlistData: [],
            pageNo: 1));

  @override
  Stream<WishlistProductsState> mapEventToState(
      WishlistDetailEvent event) async* {
  ...
  Future<WishlistProductsState> _mapProductsFetchedToState(
      WishlistProductsState state) async {
  ...
  Future<WishlistProductsState> _mapProductsRefreshToState(
      WishlistProductsState state) async {
    if (state.hasReachedMax) return state;
    try {
